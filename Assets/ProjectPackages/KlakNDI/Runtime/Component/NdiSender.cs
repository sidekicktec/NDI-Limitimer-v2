...
public sealed partial class NdiSender : MonoBehaviour
{
    ...
    void PrepareSenderObjects()
    {
        ...
        if (_send == null && captureMethod == CaptureMethod.FullView)
            _send = SharedInstance.FullViewSend;
    ...
    void ReleaseSenderObjects()
    {
        ...
        if (SharedInstance.IsFullViewSend(_send))
        {
    ...
    #region Capture coroutine for the Texture/FullView capture methods
    ...
    System.Collections.IEnumerator CaptureCoroutine()
    {
        for (var eof = new WaitForEndOfFrame(); true;)
        {
            ...
            if (captureMethod == CaptureMethod.FullView)
            {
